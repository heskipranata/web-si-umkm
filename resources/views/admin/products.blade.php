<x-admin-layout>
    <section class="tab-content" id="products">
        <div class="flex justify-between items-center mb-6">
            ...
            <a id="btnOpenAdd"
                class="bg-green-600 hover:bg-green-700 px-5 py-2 rounded text-white font-semibold transition"
                href="{{ route('menus.create') }}">+ Tambah Produk</a>
        </div>
                            ...
                            <td class="py-3 px-5">
                                @if ($menu->image)
                                    <img src="{{ Str::startsWith($menu->image, ['http://', 'https://']) ? $menu->image : asset($menu->image) }}"
                                        alt="{{ $menu->name }}" class="h-16 w-16 object-cover rounded" />
                                @else
    ...
    <!-- Modal Tambah Produk -->
    <div id="modal"
        class="modal hidden fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50 p-4">
        <div class="bg-white rounded-lg shadow-lg w-full max-w-lg p-6 relative">
            <h2 id="modalTitle" class="text-xl font-bold mb-4 text-green-700">Tambah Produk</h2>
            <form id="productForm" action="{{ route('menus.store') }}" method="POST" enctype="multipart/form-data"
                class="space-y-4">
                @csrf
                ...
                <div>
                    ...
                    <select id="productType" name="type" class="w-full border border-gray-300 rounded px-3 py-2"
                        required>
                ...
                <div>
                    ...
                    <input type="number" id="productPrice" name="price" min="1000"
                        class="w-full border border-gray-300 rounded px-3 py-2" required />
                </div>
                ...
                <div class="flex justify-end space-x-3 mt-6">
                    ...
                    <button type="submit"
                        class="px-4 py-2 bg-green-600 hover:bg-green-700 rounded text-white font-semibold">Simpan</button>
                </div>
            </form>
